...
def weights_initialize(shape, var_name):
	...
	w = tf.compat.v1.get_variable(xavier_initializer(shape=shape, dtype=tf.float32), name=var_name, shape=shape, dtype=tf.float32)
	return w
...
def bias_initialize(shape, var_name):
	...
	b = tf.compat.v1.get_variable(tf.zeros(shape=shape, dtype=tf.float32), name=var_name, shape=shape, dtype=tf.float32)
	return b