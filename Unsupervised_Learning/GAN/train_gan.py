...
from layers import weights_initialize, bias_initialize
from util import gen_noise, show_generated_images, save_generated_images

def cost(logits, labels):
	"""
	This function will add our cost node inside the tensorflow graph.
...
class Generator():
	...
	def __init__(self):
		with tf.variable_scope('Generator'):
			self.G_w1, self.G_b1 = weights_initialize([128, 256], 'G_w1'), bias_initialize([256], 'G_b1')
			self.G_w2, self.G_b2 = weights_initialize([256, 784], 'G_w2'), bias_initialize([784], 'G_b2')
...
class Discriminator():
	...
	def __init__(self):
		with tf.variable_scope('Discriminator', reuse=tf.compat.v1.AUTO_REUSE):
			self.D_w1, self.D_b1 = weights_initialize([784, 256], 'D_w1'), bias_initialize([256], 'D_b1')
			self.D_w2, self.D_b2 = weights_initialize([256, 1], 'D_w2'), bias_initialize([1], 'D_b2')
